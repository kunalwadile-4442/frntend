...
const DropdownSelect: React.FC<DropdownProps> = ({
  ...
  const classNames = {
    ...
    control: (props) =>
      `mt-[-8px] overflow-auto bg-transparent ${themes?.[theme]?.control} ${errors ? '!border !border-red-600' : ''
      }`,
  ...
  const customStyles = {
    control: (provided: any, state) => ({
      ...provided,
      backgroundColor: state.isDisabled ? '#fdfcfb' : 'transparent',
      padding: "0.08rem",
      border: 'none',
      boxShadow: 'none',
    }),
        ...
        <div className="w-full h-full">
          {isCreatable ? (
            ...
            <Select
              ...
              placeholder={placeholder || (defaultValue ? `Select ${defaultValue}` : '')}
              className={`${className} text-sm select-form-containers bg-transparent`}
              onFocus={callOnFocus}
              onBlur={callOnBlur}
              inputValue={searchInput}
              components={{ IndicatorSeparator: null }}
  ...
  return (
    <div className={`flex  ${inline ? 'items-center space-x-2' : 'flex flex-col'}  ${formClassName} border-[1px] rounded-full h-11  ${errors ? "border-red-600" : `focus-within:border-[#C8C9C9] border-[#C8C9C9]`}`}>
      {label && (
        <label
          className={`font-normal mt-[-10px] bg-white w-fit ml-2 dropdown-label font-${fontWeight} ${labelClassName} form-label  text-gray-600  text-sm`}
          htmlFor={name}
        >
          {label}
