...
import { setConfirmModalPopup } from "../../../../redux/actions/action";
import { useUiReducer } from "../../../../redux/getdata/useUiReducer";

const ConfirmModalPopup = (prop) => {
  const dispatch = useDispatch();
  const { ModalPopup } = useUiReducer();
  const dropdownRef = useRef<HTMLDivElement>(null);

  useEffect(()=>{
    CloseData();
  },[]);
  const handleClose = async (any) => {
    if (ModalPopup?.callBackModal) {
        await ModalPopup?.callBackModal(ModalPopup?.data?.item);
    }
    CloseData()
  };
  const CloseDataWithResources = async () =>{
  ...
  if (ModalPopup?.show !== "CONFIRM_MODAL") {
    return null
  }
  return (
    <>
          ...
          <div className="relative p-4 w-full max-w-lg ">
            ...
            >
              <div className="flex items-center justify-between p-4  md:p-4 rounded-t">
                ...
                <button
                  onClick={CloseDataWithResources}
                  className="confirmation-popup-close-btn text-gray-400 bg-transparent hover:bg-gray-200 hover:text-primary rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center dark:hover:bg-primary dark:hover:text-white"
                >
                  <svg
                    className="w-3 h-3"
                    aria-hidden="true"
              ...
              <div className="flex items-center justify-center p-3 md:p-3 mt-2">
                {ModalPopup?.buttonSuccess &&(
                  <button
                    type="button"
                    onClick={handleClose}
                    className="warnpopup-button text-white bg-primary hover:bg-primary-100  focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary dark:hover:bg-primary-100"
                  >
                    {ModalPopup?.buttonSuccess}
                  </button>
                )}
                {ModalPopup?.buttonCancel &&(
                  <button
                    onClick={ModalPopup?.callBackCancelModal?CloseDataWithResources:CloseData}
