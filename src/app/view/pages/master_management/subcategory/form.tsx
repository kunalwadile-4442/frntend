...
import { ISubCategoryListTypes } from "../../../../redux/modules/master_management/subcategory";
...
const AddSubCategoryForm = () => {
  ...
  const onSubmit = (data: ISubCategoryListTypes) => {
    setLoader(true);
    if (path === "add") {
      const payload = {
        ...data,
  ...
  return (
    <FormLayout
      ...
      onSubmit={onSubmit}
      content={formContent(
        path === "add" ? "Add Sub Category" : "Edit Sub Category",
        "",
        path === "add" ? "Add Sub Category" : "Edit Sub Category"
      )}
    ...
    >
      <div className="grid grid-cols-1 row-margin">
        <InputField
          ...
          className="mx-2 mt-4"
          register={register("sub_category_name", {
            required: "Sub Category Title is required",
            maxLength: {
              value: 255,
              message: "Sub Category Title cannot exceed 255 characters",
            },
          })}
